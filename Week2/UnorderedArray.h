...
public:
		...
		m_array(NULL), M_MAXsIZE(0), m_growSize(0), m_numElements(0)
	{
		if (size)
		{
			...
			memset(m_array, 0, sizeof(T) * m_maxSize); //Explicity allocating memory based on type T
	...
	//Insertions
	//Fast insertion for an UnorderedArray -- Big-O is O(1)
	void push(T val)
	{
		assert(m_array != NULL); //Debugginh purposes
